...
# ///
#
# Vendored from Trail of Bits' skills-curated repository.
#
#   Source:  https://github.com/trailofbits/skills-curated/blob/main/scripts/scan_plugin.py
#   Author:  Trail of Bits (https://github.com/trailofbits)
#   License: CC-BY-SA-4.0
#            https://creativecommons.org/licenses/by-sa/4.0/
#            trailofbits/skills-curated is licensed under Creative Commons
#            Attribution-ShareAlike 4.0 International.
#
# Vendored into mbailey/skills on 2026-05-16 (SKF-8). Logic is unchanged from
# upstream; the only modifications are this attribution header and whitespace
# reformatting applied by this repo's ruff-format pre-commit hook. Per
# CC-BY-SA-4.0, this file and any derivative modifications to it remain
# licensed under CC-BY-SA-4.0; keep this notice and attribute Trail of Bits if
# you redistribute or adapt it.
#
...
def markdown_code_ranges(lines: list[str]) -> frozenset[int]:
    ...
    for i, line in enumerate(lines):
        ...
        if fence_start is None:
            ...
            if (
                close
                and all(c == fence_char for c in close)
                and len(close) >= fence_len
            ):
...
def scan_file(path: Path, rel_path: str) -> list[Finding]:
    ...
    for idx, line in enumerate(lines):
        ...
        findings.extend(
            check_credential_access(line, idx, rel_path, suffix, code_lines)
        )
